...
hatenadfp.displayAdsWithContentMatch = function (config) {
    ...
    for (var i = 0; i < config.contentMatchConfigs.length; i++) {
        ...
        var regexp = hatenadfp._regExpFromKeywords(keywords);
        scoresOfConfigs[i] = 0;
...
hatenadfp._enhanceKeywords = [ 'Ruby', 'SE', 'SI', 'iOS', 'iPhone', 'CentOS', 'Android', 'Mac', 'Java', '英語', 'JavaScript', 'Qiita', 'Linux', 'Git', 'Chrome', '転職', '自動車', 'PHP', 'Python', 'HTML', 'Twitter', 'MySQL', 'GitHub', 'CSS', 'jQuery', 'Ubuntu', 'Rails', '資格', 'Vim', 'SSH', 'Perl', 'C#', 'Apache', '正規表現', 'SIer', 'Scala', 'ERP', 'Swift', 'HTML5', 'Facebook', 'Emacs', 'Node.js', 'nginx', 'アジャイル', 'オフショア', '情報システム', 'Xcode', 'SAP', 'Haskell', 'CSS3', 'Zsh', 'Heroku', 'Objective-C', 'MongoDB', 'デスマーチ', 'MacOSX', 'Scheme', 'CoffeeScript', '要件定義', 'ウォーターフォール', 'オフショア開発', 'システムインテグレーター', '人月', '上流工程', '工数管理', 'システム統合' ];
...
hatenadfp._assignAdTargetingSegments = function() {
    ...
    var adwordsKeywordRegExp = hatenadfp._regExpFromKeywords(adwordsKeywords);
    ...
    var enhanceKeywordRegExp = hatenadfp._regExpFromKeywords(hatenadfp._enhanceKeywords);
    ...
    for (var tag in hatenadfp._synDotPMPTagKeywordsMap) {
        ...
        var synDotKeywordRegExp = hatenadfp._regExpFromKeywords(escapedKeywords);
