...
//          buyPrice = this.getString(this.Now, self.marketPlace);
 ...
 var hAndW0=_amzn_utils.getTextHeightAndWidth(buyButtonStr, "100% Verdana, Arial, Helvetica, sans-serif","10", true, "", self);
    ...
    // forester call
    fCall: function(marketPlace, map) {
        var json = JSON.stringify(map);
        (new Image()).src = "//" + this.getString(this.AssocForesterVIP, marketPlace) + "?cb=" + (new Date()).getTime() +
                            "&p=" + encodeURIComponent(json);
    },

    recordImpression: function(tag, linkCode,  marketPlace, pandaCompliant) {
        var orgUnit = 1;
        switch(marketPlace) {
        ...
        (new Image()).src = '//' + this.getString("PandaAssocImpVIP", marketPlace) + '/e/ir?'
                + 'o=' + orgUnit
                + '&t=' + tag
                + '&l=' + linkCode;

        this.fCall(marketPlace,
                   {program: orgUnit, tag: tag, linkCode: linkCode,
                    refUrl: ((window.location != window.parent.location)?document.referrer:document.location.href),
                    panda: pandaCompliant || false});
        if(marketPlace == 'US') {
            if(prefix != 'https') {
    ...
    AssocForesterVIP: 'AssocForesterVIP',
    AmazonCom: 'AmazonCom',
...
function main() {
    var wdgt = {
        ...
        showPSABanner: function(){
            var widget_div = _amzn_utils.$('amazon_widget_'+ this.marketPlace + '_' + this.templateId + '_' + this.instanceId);
            if(this.showPSA){
                var div = _amzn_utils.$('amzn_wdgt_t_'+this.templateId+'_'+this.instanceId);
                var divHeight = div.clientHeight;
                var divWidth = div.clientWidth;
                var fontSize = "2.2em";
                if(((divWidth<250) || (divWidth < 500 && divHeight < 200 ))){
                    fontSize = "1.5em";
                }
                if((divWidth < 350 && divHeight < 130)){
                    fontSize = "1em";
                }
                if((divWidth == 120 && divHeight == 600)){
                      fontSize = "1.2em";  
                }
                var pandaImage = _amzn_utils.amznMediaserverURL(this.marketPlace) + "/20070822/"+this.marketPlace.toUpperCase()+"/img/panda-bg-sprite.png";
                widget_div.innerHTML = '<div id="pandaText" style="background-image:url('+pandaImage+');display:table-cell;color:white;font-weight: 800;vertical-align:middle;text-align:center;font-family:sans-serif;padding-left:10%;padding-right:10%;user-select: none;-webkit-touch-callout: none;-webkit-user-select: none;background-position: center;line-height:1.3em;width:'+divWidth+'px;height:'+divHeight+'px;font-size:'+fontSize+'">' +
                    'Let\'s <span style="color:red">Fight Hunger </span> Together' +
                '</div>';
            }
        },
	...
	isIAB:false,
        ...
        mergeTemplate: function() {
            ...
            if(this.showPSA){
                headerStart += ';visibility:hidden';
            }

            headerStart += '\'> <div class=\'amzn_wdgt_pad\' style=\'' + this.style_wdgt_pad() + '\'>'
...
marketPlace: 'JP',/*Locale_specific_stuff_local_END*/
...
/*Theme_specific_stuff_END*/
    ...
    if (typeof amzn_wdgts_vars[wdgt.marketPlace][wdgt.templateId][id] != 'undefined') {
        ...
        _amzn_utils.recordImpression(amzn_wdgts_vars[wdgt.marketPlace][wdgt.templateId][id].tag, wdgt.linkCode,  wdgt.marketPlace, wdgt_params.pandaCompliant);
...
var curr_mkt_place = 'JP';/*Locale_specific_stuff_global_END*/
