...
def delete_checksum(path, read_file):
    with codecs.open(str(path) + '/' + str(read_file) + '_gnss.log', 'r', 'Shift-JIS', 'ignore') as ld:
        lines = ld.readlines()
    ld.close()
...
def log_filter(path, read_file , m):
    ...
    f = open(str(path) + '/' + str(read_file) + '_gnss_' + str(m.lower()) + '.log', 'w')
    for line in lines:
...
def gsv_filter(path, read_file, nmea):
    ...
    for fname, regex in zip(fnames, regexes):
        f = open(str(path) + '/' + str(read_file) + '_gnss_' + str(nmea.lower()) + str(fname), 'w')
        for line in lines:
...
unique_col_names = [col_glgsv, col_gpgga, col_gpgsv, \
